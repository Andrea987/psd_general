...
try:
    ...
    load_boston = None  # removed since scikit-learn 1.2; only an error if 'boston' is requested
...
def dataset_loader(dataset):
    ...
    if dataset in DATASETS:
        if dataset == 'iris':
            ...
            assert load_boston is not None, (
                "load_boston was removed from scikit-learn (>=1.2) and is not available in this "
                "environment; see the ImportError raised by sklearn.datasets for details/alternatives."
            )
            X = load_boston()['data']
